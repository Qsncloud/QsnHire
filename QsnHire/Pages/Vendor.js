...
export default function Vendor() {
  ...
  const triggerRefresh = async () => {
    setIsRefreshing(true); // Set isRefreshing to true before starting refresh
    setTimeout(() => {
      setIsRefreshing(false); // Set isRefreshing to false after refreshing
    }, 6000);
     ...
     {offline ? (
      ...
      <Text style={styles.buttonText}>Retry</Text>
        ...
        <View   style={{ flex: 1}}> 
          ...
          {isLoading && <ActivityIndicator style={styles.loadingContainer} size="large" color="#2a0a2b" animating={true}/>}
          <WebView
          style={{ flex: 1}}
            source={{  uri: lastVisitedUrl || 'https://qsnhire.com/home/signup' }}
            ref={webViewRef}
            originWhitelist={['*']}
            ...
            onShouldStartLoadWithRequest={(request) => {
              ...
              // Check if the URL is an external link (not part of your app)
              if (!url.startsWith('https://qsnhire.com/home/signup')) {
                // Open the URL in the device's default browser
                Linking.openURL(url);
                return false; // Prevent the WebView from loading the URL
              }
              return true; // Allow the WebView to load URLs within your app
            }}
          />
